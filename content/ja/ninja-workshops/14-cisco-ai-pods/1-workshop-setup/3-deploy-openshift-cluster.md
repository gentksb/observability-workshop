...
> `g5.2xlarge` インスタンスタイプを指定していることに注意してください。このインスタンスタイプには、ワークショップの後半で使用する NVIDIA GPU が含まれています。このインスタンスタイプは比較的高価で、執筆時点で1時間あたり約 $1.21 です。また、2つのレプリカを要求しているため、クラスターの稼働時間に注意してください。コストは急速に蓄積されます。

クラスターの準備が完了したかどうかを確認するには、以下を実行します。
...
> 注意: `rosa describe cluster -c $CLUSTER_NAME` コマンドを実行し、結果として得られた API Server URL を以下のコマンドに代入してから実行してください。例えば、サーバー名は `https://api.rosa-test.aaa.bb.openshiftapps.com:443` のようになります。
