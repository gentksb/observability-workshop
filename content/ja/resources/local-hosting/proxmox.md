---
title: Proxmox でのローカルホスティング
weight: 3
description: Proxmox VE でローカルホスティング環境を構築する方法を学びます
---
...
`ubuntu-cloud-k3d.sh` スクリプトは、Proxmox VE 上での Splunk Observability Workshop VM の作成を自動化します。必要なツールと設定がすべてプリインストールされた、Ubuntu 24.04 cloud-init ベースの完全な VM を作成します。

#### 前提条件

- 管理者アクセス権を持つ Proxmox VE クラスター
- クラウドイメージとパッケージをダウンロードするためのインターネット接続
- 利用可能な VM ID 範囲とストレージ容量
- ワークショップアクセス用の有効な SWiPE ID
- **ローカルストレージで Snippets を有効化** - cloud-init 設定ファイルに必要です。有効化するには：
...
- ユーザー確認と SWiPE ID 入力のための対話式プロンプトを表示します

#### 2. 認証と設定

- Splunk workshop API に対して SWiPE ID を検証します
...
- Splunk Observability Workshop の教材をダウンロードします
- ワークショップトークンを使用して Kubernetes シークレットをセットアップします
...
- **デプロイが遅い**: cloud-init がすべてのインストールを完了するまで追加の時間を見込んでください

### タグ

作成された VM には以下のタグが付けられます：`o11y-workshop`、`noble`、`k3d`
